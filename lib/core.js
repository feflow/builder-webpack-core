...
class BuilderCore {
    ...
    // 当前运行的时候的根目录
    static projectRoot = Config.getPath();

    // 最基础的配置
    static baseConfig = {
        entry: glob.sync(path.join(this.projectRoot, './src/pages/*')),
        module: {
            rules: []
        },
        plugins: [],
        resolve: {
            alias: glob.sync(path.join(this.projectRoot, './src/*/')) // 支持Webpack import绝度路径的写法
        },
        resolveLoader: {}
    };

    static setHappyPack(jsLoader) {
